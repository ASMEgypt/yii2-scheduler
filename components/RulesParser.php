...
use DHTMLX_Scheduler\RecurringType;
use yii\base\Component;

class RulesParser extends Component
{
    public $rule = null;
    public $duration = null;
    public $startDate = null;
    public $endDate = '9999-02-01 00:00:00';
    public $currentDate = null;
    public $checkDate = null;
    public function check() {
        $schedulerDate = $this->getSchedulerDate();
        $startDate = strtotime($this->checkDate);
        $schedulerDate->transpositor($startDate);
        if ($result = $schedulerDate->date_generator($startDate, strtotime($this->currentDate))) {
            return -1;
        } else {
            $startDate = strtotime($this->checkDate) - $this->duration;
            $schedulerDate->transpositor($startDate);
            if ($schedulerDate->date_generator($startDate, strtotime($this->currentDate))) {
                return 0;
            } else {
                return 1;
            }
    ...
    protected function getSchedulerDate() {
        $data = [
            'start_date' => $this->startDate,
            'end_date' => $this->endDate,
            'text' => '',
            'rec_type' => $this->rule,
            'event_length' => $this->duration,
            'event_id' => 0,
            'event_pid' => 0,
        ];
        $date = new SchedulerDate($data, [], $this->startDate);
        return $date;
    }

    protected function getRecurringType() {
        $startTimeStamp = strtotime($this->startDate);
        $endTimeStamp = strtotime($this->endDate);
        $type = new RecurringType($this->rule, $startTimeStamp, $endTimeStamp, $this->duration, [
